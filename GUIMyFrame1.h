...
class GUIMyFrame1 : public MyFrame1 {
   ...
   public:
    ...
    wxImage img;
    wxImage imgResult;    // Kaleidoscope
    wxImage imgResized;   // resized to panel size
    wxImage imgOriginal;  // original not modified
    ...
    void drawAxis();
    std::function<wxImage(const wxImage&, double)> getRotateFunc();
    void drawKaleidoscope(
        std::function<wxImage(const wxImage&, double)> rotateFunc,
        double angle);
    void translateXY(double x, double y);
    void resetOptions();
    static float cubicInterpolate(float p[4], float x);
    static float bicubicInterpolate(float patch[4][4], float x, float y);
    static wxImage RotateImageCubic(const wxImage& source, double angleDegrees);
    static wxImage RotateImageLinear(const wxImage& source,
                                     double angleDegrees);
